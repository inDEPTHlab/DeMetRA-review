...
class UpSet:
    ...
    def __init__(
        ...
        self.color_map = color_map or matplotlib.cm.get_cmap("tab10")
        self.subset_colors = None

        if color_by is not None:
            if isinstance(data, pd.DataFrame):
                # Each subset: get the color_by value (e.g., most common, or first)
                color_vals = self._df[color_by]
                # If multiple rows per subset, choose representative (e.g., mode)
                subset_to_color = color_vals.groupby(self._df["_bin"]).agg(lambda x: x.iloc[0])
            else:
                # If Series, color_by must be a mapping from subset idx to color
                subset_to_color = pd.Series(self.intersections.index.get_level_values(color_by), index=self.intersections.index)
            # Map each value to a color
            unique_vals = subset_to_color.unique()
            val_to_color = {val: self.color_map(i / max(1, len(unique_vals)-1)) for i, val in enumerate(unique_vals)}
            # Store color for each subset in the order of intersections
            self.subset_colors = subset_to_color.map(val_to_color).values.tolist()
        else:
            self.subset_colors = [facecolor] * len(self.intersections)
    ...
    def plot_matrix(self, ax):
        ...
        # Use subset_colors if available, otherwise fallback
        dot_colors = self.subset_colors if self.subset_colors is not None else [self._facecolor] * len(data)

        # Prepare styling
        styles = [
            [
                dict(self.subset_styles[i], facecolor=dot_colors[i])
                if inclusion[i, j]
    ...
    def plot_intersections(self, ax):
        ...
        rects = self._plot_bars(
            ax, self.intersections, title="Number of MPSs", colors=self.subset_colors
        )
...
def plot(data, fig=None, color_by=None, color_map=None, **kwargs):
    ...
    return UpSet(data, color_by=color_by, color_map=color_map, **kwargs).plot(fig)
